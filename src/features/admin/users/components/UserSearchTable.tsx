// src/features/admin/users/components/UserSearchTable.tsx
...
import type { PageMetadata } from '@/types/transactionSearch';
import type { UserSummary } from '@/types/user';
import { formatISOTimestampAsLocalDate } from '@/utils/dates';
import { cn } from '@/utils/cn';
import { columnWidthClass } from '@/utils/columnWidth';
import { PAGE_SIZE_OPTIONS, type PageSize } from '@/features/admin/users/utils/urlState';
...
/**
 * Translate the URL sort representation (['createdAt,DESC', 'id,DESC']) into
 * TanStack Table's SortingState ([{ id: 'createdAt', desc: true }, ...]).
 */
  ...
  const columns = useMemo<ColumnDef<UserSummary>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => <div className="truncate">{row.original.displayName}</div>,
        size: 180,
        minSize: 150,
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return <div className="truncate">{roles.join(', ')}</div>;
        },
        size: 150,
        minSize: 120,
      },
      {
        ...
        cell: ({ row }) => (
          <span className="whitespace-nowrap">
            {formatISOTimestampAsLocalDate(row.original.createdAt)}
          </span>
        ),
        size: 120,
        minSize: 120,
      ...
      },
      {
        accessorKey: 'id',
        header: () => (
          <SortableHeader label="ID" field="id" sort={sort} onSortChange={onSortChange} />
        ),
        cell: ({ row }) => {
          const id = row.original.id;
          const truncated = id.length > 6 ? `…${id.slice(-6)}` : id;
          return (
      ...
      {!showInitialSkeleton && totalElements > 0 && (
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="text-sm text-muted-foreground">
            Showing {showingFrom}–{showingTo} of {totalElements.toLocaleString()} users
          </div>
